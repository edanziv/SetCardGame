...
public class Dealer implements Runnable {
    ...
    public void run() {
        ...
        while (!shouldFinish()) {
            ...
            for (int i = 0; i < players.length; i++) {
                players[i].setAvailability(true); //allow players to see the table
            }
        ...
        if(!firstLoop){
            for (int i = 0; i < players.length; i++) {
                if(!players[i].isHuman()){
                    ...
                    players[i].getThread().interrupt(); //redundant
                }
            }
        }
        firstLoop=false;
    ...
    public void terminate() { 
        for (int i = players.length-1; i >=0 ; i--) {
            ...
            if(!players[i].isHuman()){players[i].getAiThread().interrupt();}
    ...
    private void setTest(Player player){
        if(player.tokensCounter==3){
            ...
            for (int i = 0; i < cards.length; i++) {
                ...
                int token = player.tokensOnTable[0][i];
                Integer tableCard = null;
                if(token != -1){tableCard = table.slotToCard[player.tokensOnTable[0][i]];}
